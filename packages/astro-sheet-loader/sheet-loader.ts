...
import { z, ZodTypeAny } from "astro/zod";
import type {
  JSONData,
  Cell,
...
async function parseJSON(text: string, url?: string): Promise<JSONData> {
  ...
  if (jsonObject.status == "error") {
    throw new AstroError(
      `Error in JSON data for '${url}': ${jsonObject.errors?.[0].detailed_message}`,
    );
  }
  return jsonObject;
}
...
export async function sheetSchemaToZodSchema({
  ...
  allowBlanks = false,
}: sheetSchemaToZodSchemaOptions): Promise<z.ZodObject<any>> {
...
function valueOrFormat(c: Cell | null): string | number | boolean | undefined {
  ...
  if (!c) {
    ...
    return c.v; // strings
  } else if (Number.isInteger(c.v) && regexISODate.test(c.f)) {
    return c.f; // iso dates
  } else if (regexFunDate.test(`${c.v}`)) {
